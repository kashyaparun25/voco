...
//!
//! **Targeting:** when the caller captured the PID that owned keyboard focus
//! at dictation start (see [`crate::services::focus::focused_app_pid`]), the
//! CGEvents are posted directly to that process with `CGEventPostToPid`
//! (FluidVoice's pattern) — the text lands in the app the user dictated into
//! even if focus has since drifted, and it is more reliable for floating
//! launchers than the system HID tap. The HID tap remains the fallback.
//!
...
/// Post a keyboard CGEvent either straight to a process or to the HID tap.
fn post_key_event(event: &CGEvent, target_pid: Option<i32>) {
    match target_pid {
...
impl TextInjector {
    ...
    ///
    /// `target_pid` is the PID that owned keyboard focus when dictation
    /// started; when present, injection events are posted directly to it,
    /// with the system HID tap as the fallback destination.
    pub fn inject(text: &str, target_pid: Option<i32>) -> anyhow::Result<()> {
        ...
        info!(
            "Injecting {} characters of text (target pid: {:?})...",
            text.len(),
            target_pid
        );

        // 1. Clipboard-free unicode typing, posted to the captured PID first.
        if target_pid.is_some() {
            match Self::inject_unicode_events(text, target_pid) {
                Ok(()) => {
                    info!("Text injected (clipboard-free unicode events → PID {:?}).", target_pid);
                    return Ok(());
                }
                Err(e) => warn!("PID-targeted unicode injection failed ({e}); trying HID tap."),
            }
        }
        match Self::inject_unicode_events(text, None) {
            Ok(()) => {
                info!("Text injected (clipboard-free unicode events, HID tap).");
                return Ok(());
            }
            Err(e) => warn!("Unicode-event injection failed ({e}); falling back to clipboard paste."),
        }

        // 2. Clipboard + native ⌘V (PID-targeted when possible).
        match Self::inject_clipboard_paste(text, target_pid) {
            Ok(()) => {
    ...
    fn inject_unicode_events(text: &str, target_pid: Option<i32>) -> anyhow::Result<()> {
        ...
        while start < utf16.len() {
            ...
            post_key_event(&down, target_pid);

            let up = CGEvent::new_keyboard_event(source.clone(), 0, false)
                .map_err(|_| anyhow::anyhow!("failed to create key-up event"))?;
            post_key_event(&up, target_pid);
    ...
    fn inject_clipboard_paste(text: &str, target_pid: Option<i32>) -> anyhow::Result<()> {
